...
object Huffman {
   ...
   */
    def until[T](xxx: List[T]=>Boolean, yyy: (List[T] => List[T]))(zzz: List[T]): List[T] = if(xxx(zzz)) zzz else until(xxx,yyy)(yyy(zzz))
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =  a.map(x=> (x._1,List(0):::x._2 )) ::: b.map(x=>(x._1,List(1):::x._2))
